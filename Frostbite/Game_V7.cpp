...
int main()
{
	...
	while (!WindowShouldClose())		// run the loop until the user presses ESCAPE or presses the Close button on the window
	{
		...
		case GAMEPLAY:
		{
			...
			DrawRectangle(screenWidth / 2.0f, 0, 30, 30, ORANGE);
			...
			//dufault 1280 x 800
			DrawText(TextFormat("FPS: %i", GetFPS()), 100, 20, 20, WHITE);
			DrawText(TextFormat("Score: %i", stats.score), 600, 775, 20, WHITE);
			DrawText(TextFormat("Round: %i", stats.round), 600, 750, 20, WHITE);
			DrawText(TextFormat("Health: %i", playerHealth), 600, 725, 20, WHITE);
			DrawText(TextFormat("Zombies Left: %i", stats.zombiesLeft), 600, 700, 20, WHITE);
			DrawText(TextFormat("Zombies Killed: %i", stats.zombiesKilled), 600, 675, 20, WHITE);
